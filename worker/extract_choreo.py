...
landmarks compatible with the app's choreography loader. This is the
worker copy; the model sits at /app/models/pose_landmarker_lite.task
inside the Docker image rather than next to the script.
"""
...
DEFAULT_MODEL = Path(
    os.environ.get("MP_MODEL_PATH", "/app/models/pose_landmarker_lite.task")
)
...
def main():
    ...
    while True:
        ...
        if idx % 240 == 0:
            ...
            print(
                f"  …{idx}/{total_frames} ({pct:.1f}%) kept {len(frames)}",
                flush=True,
            )
